...
/*
TODO Now : 
approx 2857 lines of code so far 
107 of road to learning react (an use case(typo))
1. Sort completed roll calls in history
2. Assign Present and Absent in history
3. Add functionality for upgrade in settings
4. Add functionality to change logo from account in settings
5. Add alert for the student who has been undoed
6. Add functionality to display days each member came for a meeting.
7. Classname vs class
8. Add shortcut to add new name in material table
9. Bug:When search is used to make a student present and done is clicked,
only that student is diplayed in the table.
10. Use page redirection to direct user to homepage after logging in.
11. In edit, code to prevent empty input only works for a single whitespace
12. Add 404 page with either Django or node.js
13. Sort student roll call attendance in home page in ascending and descending order 
14. Create my own file format and enable export of data to another pc
15.URGENT: ADD FUNCTIONALITY TO DISPLAY DAYS EACH STUDENT WAS PRESENT & ABSENT.
...
const ColorButton = withStyles((theme) => ({
  ...
  const Remove = (data,val)=>{

for (var i =0; i<data.length -1; i++){
    if (data[i] !== data[i+1] && !val.includes(data[i]) && !val.includes(data[i+1])){
        val.push(data[i],data[i+1])}
    if (data[i] === data[i+1] && val.includes(data[i]) && val.includes(data[i+1])){
        console.log('')}
    if (data[i] !== data[i+1] && val.includes(data[i]) && val.includes(data[i+1])){
        console.log('')}
  ...
  ChangeHouse(value){
    ...
    render(){
        ...
        return(
            ...
            || list.some(arr => arr.name === undefined) || list.some(arr=> arr.class === undefined)
            || list.some(arr => arr.name === "")
             ? "disabled":"contained" }  
            onClick={this.toggleView1} startIcon={<PlayArrow/>} className = "StartBtn"> Start Roll Call </Button>
            </div>
            </div>
        );
    }
...
class App extends Component {
    ...
    //Material Table.js Line 542 props.columns.findIndex is not a function   
        render() {
